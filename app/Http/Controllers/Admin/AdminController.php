...
class AdminController extends Controller
{
    ...
    public function unggahModul(Request $request)
    {
        ...
        // FIX-10: Validasi magic bytes ZIP, bukan hanya ekstensi
        ...
        if ($extractor->validasiModul($zipPath)) {
            // FIX-7: Sanitasi nama folder — hanya izinkan karakter alfanumerik
    ...
    public function buatBackup(Request $request)
    {
        ...
        // FIX-9: Jangan kembalikan detail error ke client
        \Log::error('[buatBackup] Gagal backup', ['error' => $result['error'] ?? '']);

        return response()->json([
            'berhasil' => false,
            'pesan'    => 'Gagal membuat backup. Silakan cek log server.'
        ], 500);
    }
    ...
    {
        // FIX-2: Cegah path traversal — basename + validasi format nama file
        $file = basename($file);
        if (!preg_match('/^backup-[\d-]+\.sql(\.gz)?$/', $file)) {
    ...
    public function hapusBackup(Request $request)
    {
        ...
        if (!preg_match('/^backup-[\d-]+\.sql(\.gz)?$/', $file)) {
            return response()->json(['berhasil' => false, 'pesan' => 'Nama file tidak valid.'], 400);
        }
        $path = storage_path('app/backups/' . $file);
        if (File::exists($path)) {
            File::delete($path);
    ...
    {
        // FIX-3: Hapus fallback base_path() + tambahkan validasi nama file
        $file = basename($request->file);
        if (!preg_match('/^backup-[\d-]+\.sql(\.gz)?$/', $file)) {
            return response()->json(['berhasil' => false, 'pesan' => 'Nama file tidak valid.'], 400);
        }
        ...
        if (!File::exists($path)) {
            // FIX-3: Tidak ada fallback ke base_path() — hanya cari di folder backups
            return response()->json(['berhasil' => false, 'pesan' => 'File backup tidak ditemukan.'], 404);
        }
        ...
        // FIX-9: Jangan kembalikan output mentah ke client — log ke server
        \Log::error('[restoreBackup] Gagal restore', [
            'file'   => $file,
            'error'  => $result['error'] ?? '',
        ...
        return response()->json([
            'berhasil' => false,
            'pesan'    => 'Gagal restore database. Silakan cek log server.'
        ], 500);
    }
